...
class NostalgiaForInfinityNext(IStrategy):
    ...
    buy_25_protection__optimize = False
    buy_25_parameters__optimize = False
    buy_25_protection__ema_fast                 = CategoricalParameter([True, False], default=False, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__ema_fast_len             = CategoricalParameter(["26","50","100","200"], default="50", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__ema_slow                 = CategoricalParameter([True, False], default=False, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__ema_slow_len             = CategoricalParameter(["26","50","100","200"], default="100", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__close_above_ema_fast     = CategoricalParameter([True, False], default=False, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__close_above_ema_fast_len = CategoricalParameter(["12","20","26","50","100","200"], default="50", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__close_above_ema_slow     = CategoricalParameter([True, False], default=False, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__close_above_ema_slow_len = CategoricalParameter(["15","50","200"], default="200", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__sma200_rising            = CategoricalParameter([True, False], default=True, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__sma200_rising_val        = CategoricalParameter(["20","30","36","44","50"], default="20", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__sma200_1h_rising         = CategoricalParameter([True, False], default=False, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__sma200_1h_rising_val     = CategoricalParameter(["20","30","36","44","50"], default="50", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__safe_dips                = CategoricalParameter([True, False], default=False, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__safe_dips_type           = CategoricalParameter(["10","50","100"], default="10", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__safe_pump                = CategoricalParameter([True, False], default=True, space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__safe_pump_type           = CategoricalParameter(["10","50","100"], default="20", space='buy', optimize=buy_25_protection__optimize, load=True)
    buy_25_protection__safe_pump_period         = CategoricalParameter(["24","36","48"], default="24", space='buy', optimize=buy_25_protection__optimize, load=True)
    ...
    buy_25_ma_offset = DecimalParameter(0.90, 0.99, default=0.922, space='buy', optimize=buy_25_parameters__optimize, load=True)
    buy_25_rsi_14 = DecimalParameter(26.0, 40.0, default=35.0, space='buy', decimals=1, optimize=False, load=True)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_condition_24_enable.value:
         ...
         # Protections
        buy_25_protections = [True]
        if self.buy_25_protection__ema_fast.value:
            buy_25_protections.append(dataframe[f"ema_{self.buy_25_protection__ema_fast_len.value}"] > dataframe['ema_200'])
        if self.buy_25_protection__ema_slow.value:
            buy_25_protections.append(dataframe[f"ema_{self.buy_25_protection__ema_slow_len.value}_1h"] > dataframe['ema_200_1h'])
        if self.buy_25_protection__close_above_ema_fast.value:
            buy_25_protections.append(dataframe['close'] > dataframe[f"ema_{self.buy_25_protection__close_above_ema_fast_len.value}"])
        if self.buy_25_protection__close_above_ema_slow.value:
            buy_25_protections.append(dataframe['close'] > dataframe[f"ema_{self.buy_25_protection__close_above_ema_slow_len.value}_1h"])
        if self.buy_25_protection__sma200_rising.value:
            buy_25_protections.append(dataframe['sma_200'] > dataframe['sma_200'].shift(int(self.buy_25_protection__sma200_rising_val.value)))
        if self.buy_25_protection__sma200_1h_rising.value:
            buy_25_protections.append(dataframe['sma_200_1h'] > dataframe['sma_200_1h'].shift(int(self.buy_25_protection__sma200_1h_rising_val.value)))
        if self.buy_25_protection__safe_dips.value:
            buy_25_protections.append(dataframe[f"safe_dips_{self.buy_25_protection__safe_dips_type.value}"])
        if self.buy_25_protection__safe_pump.value:
            buy_25_protections.append(dataframe[f"safe_pump_{self.buy_25_protection__safe_pump_period.value}_{self.buy_25_protection__safe_pump_type.value}_1h"])
        # Non-Standard protections (add below)

        # Logic
        buy_25_logic = []
        buy_25_logic.append(reduce(lambda x, y: x & y, buy_25_protections))
        buy_25_logic.append(dataframe['rsi_20'] < dataframe['rsi_20'].shift())
        buy_25_logic.append(dataframe['rsi_4'] < self.buy_25_rsi_14.value)
        buy_25_logic.append(dataframe['ema_20_1h'] > dataframe['ema_26_1h'])
        buy_25_logic.append(dataframe['close'] < (dataframe['sma_20'] * self.buy_25_ma_offset.value))
        buy_25_logic.append(dataframe['open'] > (dataframe['sma_20'] * self.buy_25_ma_offset.value))
        buy_25_logic.append(
        ...
        # Populate
        dataframe.loc[:,'buy_25_trigger'] = reduce(lambda x, y: x & y, buy_25_logic)
        if self.buy_condition_25_enable.value:
            conditions.append(dataframe['buy_25_trigger'])
