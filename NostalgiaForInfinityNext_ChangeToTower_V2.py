...
class NostalgiaForInfinityNext_ChangeToTower_V2(IStrategy):
    ...
    def sell_quick_mode(self, current_profit: float, max_profit:float, last_candle, secondlast_candle, buy_signal_candle) -> tuple:
        ...
        if (buy_signal_candle['buy_condition_35'] == 1) or (buy_signal_candle['buy_condition_36'] == 1) or (buy_signal_candle['buy_condition_37'] == 1) or (buy_signal_candle['buy_condition_38'] == 1):
            if (last_candle['close'] < last_candle[f'atr_high{self.atr_rolling_length.value}']) & (last_candle['close'] > last_candle[f'atr_high{self.atr_rolling_length.value}']):
		...
		(last_candle[f'pm_{pmaxperiod}_{self.pmaxmultiplier.value}_{self.pmaxlength.value}_{pmaxtype}'] > last_candle[f'mavalue{self.pmaxlength.value}']) &
            ...
            if (current_profit >= 0.005) & (last_candle[f'pm_{pmaxperiod}_{self.pmaxmultiplier.value}_{self.pmaxlength.value}_{pmaxtype}'] > last_candle[f'mavalue{self.pmaxlength.value}']):
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Quick sell mode
        if not buy_signal.empty:
            sell, signal_name = self.sell_quick_mode(current_profit, max_profit, last_candle, secondlast_candle, buy_signal_candle)
...
#        dataframe['adx'] = ta.ADX(dataframe)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_params['buy_condition_35_enable']:
            ...
            item_buy_logic = []
            item_buy_logic.append(dataframe[f'pm_{pmaxperiod}_{self.pmaxmultiplier.value}_{self.pmaxlength.value}_{pmaxtype}'] <= dataframe[f'mavalue{self.pmaxlength.value}'])
            item_buy_logic.append(dataframe['close'] < dataframe[f'{self.ma_type_index_1.value}_ma_buy_bullish{self.base_nb_candles_bullish_buy.value}']* self.low_offset_bullish.value)
        ...
        if self.buy_params['buy_condition_37_enable']:
            ...
            item_buy_logic = []
            item_buy_logic.append(dataframe[f'pm_{pmaxperiod}_{self.pmaxmultiplier.value}_{self.pmaxlength.value}_{pmaxtype}'] > dataframe[f'mavalue{self.pmaxlength.value}'])
            item_buy_logic.append(dataframe['close'] < dataframe[f'{self.ma_type_index_2.value}_ma_buy_bearish{self.base_nb_candles_bearish_buy.value}']* self.low_offset_bearish.value)
            item_buy_logic.append(dataframe['ewo'] > self.ewo_high.value)
